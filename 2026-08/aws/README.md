...
``upload_part_copy`` / ``list_multipart_uploads`` / ``list_parts`` /
``complete_multipart_upload`` / ``abort_multipart_upload``)
store in-flight parts under ``data/uploads/<bucket>/<upload-id>/`` -
outside the index, so object listings never see them; ``list_multipart_uploads``
(``GET /{bucket}?uploads``) is the only API view of the in-flight uploads,
it pages like a listing (``max-uploads``, ``key-marker`` / ``upload-id-marker``,
``IsTruncated`` / ``NextKeyMarker`` / ``NextUploadIdMarker``) and is what
makes orphaned uploads cleanable through the API.  Completing an upload assembles the
